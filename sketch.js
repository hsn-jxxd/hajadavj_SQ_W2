...
let platforms = [
  ...
  { x: 80, y: 250, w: 120, h: 16 }, // left low platform
  { x: 330, y: 90, w: 140, h: 16, growFactor: 1.8, color: [200, 100, 255] }, // centre platform (grows player)
  { x: 560, y: 140, w: 120, h: 16 }, // right high platform
  { x: 160, y: 135, w: 100, h: 16 }, // left high platform
  { x: 360, y: 320, w: 110, h: 16 }, // centre low platform
  { x: 620, y: 260, w: 130, h: 16 }, // far right platform
];
...
function preload() {
  ...
  playerImage = loadImage("assets/images/character.png");
}
// ------------------------------------------------------------
...
let player = {
  ...
  baseRadius: 20, // base radius used for resizing when landing on platforms
...
const PLATFORM_COLOR = [53, 173, 107]; // green
...
function resolvePlatformCollisions() {
  for (let i = 0; i < platforms.length; i++) {
    ...
    if (overlapsHorizontally && landingOnTop) {
      ...
      if (p.growFactor) {
        player.r = player.baseRadius * p.growFactor;
      } else {
